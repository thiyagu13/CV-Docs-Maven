		...
		if(driver.findElement(By.cssSelector(".btn.blue-btn.waves-effect.continue-btn.next.gotoSolid_next2")).getText().equalsIgnoreCase("Next"))
  		{
			...
			Thread.sleep(1500);
			System.out.println("Zero Size:"+driver.findElements(By.className("notify-msg")).size());
			System.out.println("Test: "+ driver.findElement(By.className("notify-msg")).getText());
		if(driver.findElements(By.className("notify-msg")).size()!=0)
		{
			System.out.println("Test: "+ driver.findElement(By.className("notify-msg")).getText());
			if(driver.findElement(By.className("notify-msg")).getText().equalsIgnoreCase("Enter Bioburden Contribution Value between 0 - 99"))
			{
			...
			else 
			{
				...
				XSSFCell status = sheet.getRow(45).getCell(7);
	...
	public void numericSpaceCheckTest() throws InterruptedException, WriteException, IOException
	{
		if(driver.findElement(By.cssSelector(".btn.blue-btn.waves-effect.continue-btn.next.gotoSolid_next2")).getText().equalsIgnoreCase("Next"))
		...
		else 
	  	{
	  		XSSFWorkbook workbook = Utils.getExcelSheet(Constant.EXCEL_PATH_Result);
	  		XSSFSheet sheet = workbook.getSheet(testcaseSheetName);
	  		XSSFCell status = sheet.getRow(48).getCell(7);
			status.setCellValue("No BioBurden"); //Print status in excel
			Utils.writeTooutputFile(workbook); // write output to the workbook
	  	}
	}
	...
	public void numericMultiDecimalTest() throws InterruptedException, WriteException, IOException
	{
		if(driver.findElement(By.cssSelector(".btn.blue-btn.waves-effect.continue-btn.next.gotoSolid_next2")).getText().equalsIgnoreCase("Next"))
		...
		else 
	  	{
	  		XSSFWorkbook workbook = Utils.getExcelSheet(Constant.EXCEL_PATH_Result);
	  		XSSFSheet sheet = workbook.getSheet(testcaseSheetName);
	  		XSSFCell status = sheet.getRow(49).getCell(7);
			status.setCellValue("No BioBurden"); //Print status in excel
			Utils.writeTooutputFile(workbook); // write output to the workbook
	  	}
		...
		if(driver.findElement(By.cssSelector(".btn.blue-btn.waves-effect.continue-btn.next.gotoSolid_next2")).getText().equalsIgnoreCase("Next"))
  		{
			//Click Next
			Thread.sleep(500);
			WebElement Submit = driver.findElement(By.cssSelector(".btn.blue-btn.waves-effect.continue-btn.next.gotoSolid_next2"));
			Submit.click();
			Thread.sleep(1000);
